...
# ================= USERNAME =================
def suggest_username(base_username, cursor):
    while True:
        suffix = random.randint(100, 999)
        new_username = f"{base_username}{suffix}"
        cursor.execute(
            "SELECT 1 FROM responsible_adult WHERE username = ?",
            (new_username,)
        )
...
def signup():
    ...
    if cursor.fetchone():
        suggested = suggest_username(username, cursor)
        conn.close()
        return jsonify({"error": "Username exists", "suggested": suggested}), 409
